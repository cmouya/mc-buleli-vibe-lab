...
### ADR-019 — Derived tenant-safe owned Step Completion and Path Progress

| | |
|---|---|
| **Status** | Accepted |
| **Decision** | **M6.4** adds **derived tenant-safe owned Step Completion and Path Progress** from **already-persisted owned Evidence** under **I-05** (`assertEvidenceAllowsCompletion`: matching `stepId` and `passed`). Goal remains the **ownership root**. Completion/Progress is **derived** through Goal → Path → Step → Evidence. Completion is **not stored** (no Step status column, no `completed_at`, **no migration 0008**). Evidence remains the durable fact; **`POST .../steps/:stepId/evidence` retains ADR-018** (persist only; do not mutate Progress). No completion command. Path Progress aggregates Step Completion: `totalSteps`, `completedSteps` (count Steps, not Evidence rows), `progressPercent` = `0` if `totalSteps = 0` else `Math.round((completedSteps / totalSteps) * 100)`. No authoritative `currentStepId`. Authorization: `learnova.sid` → `resolveSession` → requested `organizationId` → `resolveOrganizationContext` → `resolveLearnerContext` → route `goalId` → owned Path/Step proof → owned Evidence query → derived Progress. Client `learnerId` / extra-path `organizationId` never authority. Unscoped `EvidenceRepository.getByStepId` is **not** tenant authority; an **internal** owned Evidence-by-Step query is allowed; **no** public Evidence list API. HTTP is C3 only: **`GET /api/v1/organizations/:organizationId/goals/:goalId/path/progress`** (read-only). Duplicate Evidence remains allowed; completion is ANY I-05-satisfying row (monotonic: passed-then-failed stays complete). Progress GET is side-effect free. No Path/Step/Evidence tenant columns; no UNIQUE(`goal_id`); no UNIQUE(`step_id`) on Evidence. **Sessions remain User-only.** **Confirm/generate remain public.** Golden Reference **unchanged**. Mastery, Skill catalog, LearnerState persist, Adaptive Path, Learner HTTP, and I-04 are **OUT**. **M6.4 code is not started by this ADR.** |
| **Context** | M6.3 (ADR-018) made owned Evidence durable. That write does not consume I-05. Domain I-05 and GR localStorage progression exist but are unwired to owned persist. M6.4 closes only this derived Completion/Progress read gap. |
| **Alternatives considered** | Persist Step `todo`/`current`/`done` ; mutate Evidence POST to complete ; separate completion command ; tenant columns on Step/Evidence ; public Evidence-by-Step list ; currentStep pointer ; Mastery/Adaptive Path in the same slice ; UNIQUE(`step_id`) on Evidence ; migration 0008 |
| **Consequences** | Implementation follows [`docs/m6.4-implementation-plan.md`](m6.4-implementation-plan.md) only after this ADR. Evidence stays the durable fact; Progress is computed. Revocation and currentStep require later ADRs. Learner HTTP and Mastery remain deferred. ADR-013/014/015/016/017/018 Decision rows **unchanged**. |

### ADR-020 — Organization-scoped Skill identity

| | |
|---|---|
| **Status** | Accepted |
| **Decision** | **Skill Identity architecture freeze.** A **Skill** is a **stable learning capability**, not Goal, Step, Evidence, Completion, Progress, Mastery, LearnerSkill, or Learner state. **Identity is organization-scoped** (catalog governed by Organization; Skill is **not** Goal-owned). The same label in two Organizations is **not** the same Skill. **Goal remains the ownership root** for Path, Step, and Evidence (unchanged). Goal **references** Skills as desired-outcome **requirements** (**many-to-many**, **no** target proficiency / `requiredLevel`). Step **references** Skills as **coverage** (**many-to-many**; not one Step = one Skill; no primary/secondary or weights). Evidence remains **Step-scoped**; **no** `Evidence.skillId`. Step Completion remains **I-05** and is **not** Mastery (**I-03**); a completed Step does not master linked Skills; `progressPercent` is not Mastery. **I-04 remains unimplemented**; identity + Goal requirements + Step coverage + existing Learner/Evidence/ownership are prerequisites only. No LearnerSkill/Mastery persistence; no prerequisite persistence/enforcement; no Skill versioning; no public Skill HTTP; Golden Reference `step.skill` / `skills[]` remain non-authoritative (no rewrite, no dual-write; I-08). No Path/Step/Evidence tenant columns; a future Skill aggregate **may** carry `organizationId`. **No migration number** is allocated (head remains `0007_m6_1_goal_ownership`; do not assume 0008). Organization-scoped identity does **not** prohibit a **future** platform-canonical overlay, which is **not designed or implemented now**. **Implementation is NOT STARTED by this ADR.** |
| **Context** | M6.4 (ADR-019) made owned Completion/Progress derived from Evidence. Learning intelligence still lacked durable Skill identity and Goal/Step bindings. Human review approved organization-scoped identity, many-to-many Step coverage, and explicit Goal requirements without levels. |
| **Alternatives considered** | Platform-global Skill registry ; Goal-local Skill identities ; hybrid canonical overlay now ; one Step = one Skill ; infer Goal Skills only from Path ; Goal target proficiency now ; Evidence.skillId now ; persist LearnerSkill/Mastery now |
| **Consequences** | Binding record: [`docs/skill-identity-decisions.md`](skill-identity-decisions.md). Domain C1 is published separately. **Persistence architecture** (not tables) is recorded in [`docs/skill-identity-persistence-decisions.md`](skill-identity-persistence-decisions.md). C2 implementation is **not** authorized by this ADR; see [`docs/skill-identity-persistence-implementation-plan.md`](skill-identity-persistence-implementation-plan.md). Path/Step/Evidence ownership and M6.4 derived Progress **unchanged**. Mastery, SkillGap, Adaptive Path, schema, HTTP, and GR remain out until later authorized slices. ADR-013/014/015/016/017/018/019 Decision rows **unchanged**. |

At this checkpoint:

- **FROZEN ARCHITECTURE** — ADR-020 S-01–S-21, C1 domain contracts, C2-G1–C2-G9, D-C2-01–D-C2-19, Option A (FKs + Goal-rooted Skill lookup), planned conceptual tables `skills` / `goal_skills` / `step_skills`.
- **IMPLEMENTED PERSISTENCE** — none. No `skills`, `goal_skills`, or `step_skills` tables. Migration head remains `0007_m6_1_goal_ownership`. Migration 0008 is **not allocated**. C2.1–C2.5 **NOT STARTED**.
...
| D-15 | Schéma SQL spine Goal/Path/Steps/Evidence | **Décidé en M4**. Identity island **ADR-013 / M5.1**. Sessions = M5.2. Progress = derived **M6.4 / ADR-019** (no completion schema). **Skill identity = ADR-020**. **Skill catalog persistence architecture frozen** ([`skill-identity-persistence-decisions.md`](skill-identity-persistence-decisions.md) ; D-C2-01–D-C2-19) — **tables not created** ; **no migration number**. Mastery persistence remains deferred. |
